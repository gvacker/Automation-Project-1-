...
Assignment 2:
 ...
 2. Replace text ‘NewPassword331’ in the first test with your own chosen password (2 places) - passwords should match
...
describe('This is first test suite, Gerli Vacker', () => {
    // 05.07.2024
    it.only('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')

        cy.get('input#firstName').type('Nora')
        cy.get('input#lastName').type('Bridge')

        cy.get('input[name="password"]').type('NewPassword331')
        cy.get('[name="confirm"]').type('NewPassword331')
    ...
    it('User can use only same both first and validation passwords', () => {
        ...
        cy.get('input[name="password"]').type('NewPassword331')
        cy.get('[name="confirm"]').type('NewPassword331123')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        // Assert that phone number has tooltip with message
        cy.get('input#phoneNumber').should('have.attr', 'title').should('contain', 'Add phone')
